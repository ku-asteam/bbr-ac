...
static const uint32_t totalTxBytes = 50000000;	//10000000;
static const uint32_t sendBufSize  = 2000; //2000000;
...
int main(int argc, char *argv[])
{
  ...
  for (uint8_t it = 0; it < sf; it++)
  {
  		int k = it;
		char S_FLOW[64]; // The filename buffer.
		    // Put "file" then k then ".txt" in to filename.
		std::snprintf(S_FLOW, sizeof(char) * 32, "S_FLOW_%i.txt", k);
		std::ofstream traceS_FLOW(S_FLOW, std::ofstream::out | std::ofstream::trunc);
  ...
  for (uint8_t it = 0; it < sf; it++)
  {
	...
	std::ofstream traceGLOBAL("GLOBAL_OUTPUT", std::ofstream::out | std::ofstream::trunc);
	// Always check to see if the file is open and for errors.
...
*/
        ...
        netDevices.Get (i)->SetAttribute ("ReceiveErrorModel", PointerValue (em));
...
void variateDelay (Ptr<Node> node)
{
    ...
    for(uint32_t i = 0; i < ipv4->GetNInterfaces(); i++)
    {
        ...
        Ptr<RateErrorModel> em = CreateObjectWithAttributes<RateErrorModel> (
              "RanVar", RandomVariableValue (UniformVariable (0.05, 0.05)), 	//RandomVariableValue (UniformVariable (0.1, 0.1))
              "ErrorRate", DoubleValue (0.0001));
        netDev->SetAttribute ("ReceiveErrorModel", PointerValue (em));
