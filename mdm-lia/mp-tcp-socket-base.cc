...
MpTcpSocketBase::reduceCWND (uint8_t sFlowIdx)
{
    ...
    switch ( m_algoCC )
    {
        ...
        {
        	//sFlow->pktLossTime = Simulator::Now().GetSeconds();
        	//sFlow->Wmax = sFlow->cwnd;
        	//sFlow->cwnd  = std::max (cwnd  * (1 - 0.5), 1.0);


        	NS_LOG_UNCOND("\n\n\n\n\n\n\ncurrent congestion window: "<<sFlow->cwnd);
        	double delta = 0.25;
        	if(sFlow->Wrmax < 1)
        		{
        			sFlow->Wrmax = cwnd;
        			NS_LOG_UNCOND("\n\n\n\n\n\n\nvalue of beta = "<<sFlow->beta<<" Wrmax = "<<sFlow->Wrmax<<" gama "<<gama);
        			sFlow->cwnd  = std::max (cwnd * sFlow->beta, 1.0);
        	...
        	gama = sFlow->Wrmax / cwnd;
        	if(gama > 1)
        		gama = 1;
        	if(gama < 0)
        		gama = 0;
        	sFlow->Wrmax = cwnd;
        	sFlow->beta = (gama * delta) + (1 - delta) * sFlow->beta;
        	if(sFlow->beta < 0.5)
        		sFlow->beta = 0.5;
        	if(sFlow->beta >0.9)
        		sFlow->beta = 0.9;
        	...
        	NS_LOG_UNCOND("\n\n\n\n\n\n\nnew congestion window: "<<sFlow->cwnd);



        	for (uint8_t it = 0; it < subflows.size(); it++)
        	{
        		if (it == sFlowIdx)
        		{
					int k = sFlowIdx;
					NS_LOG_UNCOND("\n\n\n"<<"Subflow "<<k<<" congestion window: "<<sFlow->cwnd<<" Wmax: "<<sFlow->Wmax<<" Wrmax: "<<sFlow->Wrmax<<" beta: "<<sFlow->beta<<" gama = "<<gama);
					char S_FLOW[64]; // The filename buffer.
						// Put "file" then k then ".txt" in to filename.
					std::snprintf(S_FLOW, sizeof(char) * 32, "beta_gama_S_FLOW_%i.txt", k);
					std::ofstream traceS_FLOW(S_FLOW, std::ofstream::out | std::ofstream::app);
					// Always check to see if the file is open and for errors.
					if (traceS_FLOW.is_open())
					{
						// If it is open we can do our writing to the file.
						// Here is a example of this.
						traceS_FLOW << Simulator::Now().GetSeconds() << "\t";

						traceS_FLOW << sFlow->beta << "\t";
    ...
    double thPut = (sFlow->cwnd * sFlow->MSS * 8) / sFlow->rtt->GetCurrentEstimate().GetSeconds();
    for (uint8_t it = 0; it < subflows.size(); it++)
    {
    	if (it == sFlowIdx)
    	{
    		...
    		char S_FLOW[64]; // The filename buffer.
    		    // Put "file" then k then ".txt" in to filename.
    		std::snprintf(S_FLOW, sizeof(char) * 32, "S_FLOW_%i.txt", k);
    		std::ofstream traceS_FLOW(S_FLOW, std::ofstream::out | std::ofstream::app);
    		// Always check to see if the file is open and for errors.
    		if (traceS_FLOW.is_open())
    		{
    			...
    			traceS_FLOW << sFlow->cwnd.Get() << "\t";

    			traceS_FLOW << sFlow->rtt->GetCurrentEstimate().GetSeconds() << "\t";

    			traceS_FLOW << thPut << "\t";

    			traceS_FLOW << getPathDelay(sFlowIdx) << "\n";
    		}
    		else
    		{
    			// If the file isn't open something went wrong. Point that out.
	...
	std::ofstream traceGLOBAL("GLOBAL_OUTPUT", std::ofstream::out | std::ofstream::app);
	// Always check to see if the file is open and for errors.
	if (traceGLOBAL.is_open())
	{
		// If it is open we can do our writing to the file.
		// Here is a example of this.
		traceGLOBAL << Simulator::Now().GetSeconds() << "\t";

		NS_LOG_UNCOND ("\n\n\nTotal congestion window: "<<totalCwnd);
		traceGLOBAL << totalCwnd << "\t";

		traceGLOBAL << getGlobalThroughput() << "\t";

		traceGLOBAL << getConnectionEfficiency() << "\n";
	}
	else
	{
		// If the file isn't open something went wrong. Point that out.
...
MpTcpSocketBase::OpenCWND (uint8_t sFlowIdx, uint32_t ackedBytes)
{
    ...
    {
    	//double C = 0.4, Ks = 0, Ts = 0, beta = 0.2;
        switch ( m_algoCC )
        {
            case RTT_Compensator:
            ...
            case Linked_Increases:
            {
                ...
                //CUBIC
                //Ks = std::pow (((sFlow->Wmax * beta) / C) , (1.0 / 3));
                //Ts = Simulator::Now().GetSeconds() - sFlow->pktLossTime;
            	//increment = std::max(C * std::pow((Ts - Ks), 3) + sFlow->Wmax - cwnd, 1.0 / cwnd);
                ...
                //	Ks = std::pow (((sFlow->Wmax * beta) / C) , (1.0 / 3));
                //else
                //	Ks = std::pow (((totalCwnd * beta) / C) , (1.0 / 3));
                //Ts = Simulator::Now().GetSeconds() - sFlow->pktLossTime;
            	//increment = std::max(C * std::pow((Ts - Ks), 3) + sFlow->Wmax - cwnd, 1.0 / cwnd);

                //MPCUBIC
                //Ks = std::pow (alpha * sFlow->Wmax / (std::min(alpha*C , C)), (1.0 / 3));
                //Ts = Simulator::Now().GetSeconds() - sFlow->pktLossTime;
                //increment = std::max(C * std::pow((Ts - Ks), 3) + sFlow->Wmax - cwnd, 1.0 / cwnd);


            	//NS_LOG_UNCOND ("\n\n\ntotalCwnd = "<<totalCwnd<<"; Ks = "<<Ks<<"; Ts = "<<Ts<<"; increment = "<<increment);
    ...
    double thPut = (sFlow->cwnd * sFlow->MSS * 8)/rtt;
    for (uint8_t it = 0; it < subflows.size(); it++)
    {
    	if (it == sFlowIdx)
    	{
    		int k = sFlowIdx;
    		NS_LOG_UNCOND("\n\n\n"<<"Subflow "<<k<<" congestion window: "<<sFlow->cwnd<<" Wmax: "<<sFlow->Wmax<<" Wrmax: "<<sFlow->Wrmax<<" beta: "<<sFlow->beta);
    		char S_FLOW[64]; // The filename buffer.
    		    // Put "file" then k then ".txt" in to filename.
    		std::snprintf(S_FLOW, sizeof(char) * 32, "S_FLOW_%i.txt", k);
    		std::ofstream traceS_FLOW(S_FLOW, std::ofstream::out | std::ofstream::app);
...
MpTcpSocketBase::calculate_alpha ()
{
   ...
   for (uint32_t i = 0; i < subflows.size() ; i++)
   {
     ...
     double tmpi = std::pow (sFlow->scwnd, (4.0 - k)) / (rtt * rtt * rtt * rtt);
     if (maxi < tmpi)
       maxi = tmpi;
   ...
   if (!sumi)
     return;
   alpha = meanTotalCwnd * maxi / (sumi * sumi * sumi * sumi);
   */
